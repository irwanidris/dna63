...
import 'package:socialv/models/story/story_reaction_model.dart';
...
Future<LoginResponse> loginUser({required Map request, required bool isSocialLogin, bool setLoggedIn = true}) async {
  ...
  if (response.profilePicture.isNotEmpty) {
    ...
    userStore.setLoginAvatarUrl(AppImages.defaultAvatarUrl);
  }
...
Future<void> logout(BuildContext context, {bool setId = true}) async {
  ...
  searchFragStore.recentMemberSearchList.clear();
  searchFragStore.recentGroupsSearchList.clear();
...
Future<CommonMessageResponse> updatePhoneNumber({required Map request}) async {
  return CommonMessageResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('${APIEndPoint.membersEndPoint}/${APIEndPoint.settingsEndPoint}/${APIEndPoint.profileEndPoint}', method: HttpMethod.POST, request: request),
    ),
...
Future<DeleteAvatarResponse> deleteAvatarImage({required String id, bool isGroup = false}) async {
  log("delete");
  log("$id");
  log("${isGroup ? APIEndPoint.buddyPressGroupEndpoint : APIEndPoint.getMembers}/$id/${APIEndPoint.avatarImage}");
...
Future<ReactionCountModel> getUsersReaction({
  ...
  return ReactionCountModel.fromJson(
    await handleResponse(await buildHttpResponse('${isComments ? APIEndPoint.commentsReaction : APIEndPoint.activityReaction}/$id?page=$page&per_page=$PER_PAGE')),
  );
}
...
Future<List<SignupFields>> getProfileFields() async {
  Iterable it = await handleResponse(
    await buildHttpResponse(
      '${APIEndPoint.membersEndPoint}/${APIEndPoint.settingsEndPoint}/${APIEndPoint.profileEndPoint}/${APIEndPoint.fieldsEndPoint}?user_id=${userStore.loginUserId}',
      method: HttpMethod.GET,
    ),
  );
...
Future<GeneralSettingModel> generalSettings() async {
  ...
  if (value.pluginModel != null) {
    await appStore.setPluginStatus(value.pluginModel!);
  } else {
    log('⚠️ pluginModel is null in API response');
  }

  await appStore.setShowMembership(value.isPaidMembershipEnable);
  ...
  appStore.setGamipressRankEndPointList(value.gamipressRankTypes.validate());
    ...
    await sendMultiPartRequest(
      ...
      onSuccess: (data) async {
        ...
        status == null
            ...
            : status == "draft"
                ? toast("Story has been successfully saved to drafts")
                : toast("Story has been successfully published");
        if (index == fileList.length - 1) {
          // Only close after last story is uploaded
          appStore.setLoading(false);
          finish(context, true);
          finish(context, true);
        }
      ...
      onError: (error) {
        ...
        try {
          ...
          final parsedError = error is String ? jsonDecode(error) : error;

          if (parsedError is Map && parsedError.containsKey('message')) {
            toast(parsedError['message'], print: true); // Shows only the message
          } else {
...
Future<UserStoryData?> getUserStory() async {
  try {
    var res = UserStoryData.fromJson(await handleResponse(
      await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.storiesEndPoint}/${APIEndPoint.userStoryEndPoint}'),
    ));
    userStore.setLoginAvatarUrl(res.avatarUrl.validate());
    appStore.setLoading(false);
    return res;
  } catch (e) {
    appStore.setLoading(false);
...
Future<List<ReactionList>> getReactionList({required List<ReactionList> reactionList}) async {
  try {
    final response = await buildHttpResponse(getEndPoint(
      endPoint: '${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.storiesEndPoint}/${APIEndPoint.reactionListEndPoint}',
      params: [],
    ));
    final responseData = await handleResponse(response);
    if (responseData is List) {
      reactionList.clear();
      reactionList.addAll(responseData.map((item) => ReactionList.fromJson(item)).toList());
    }
    return reactionList;
  } catch (e) {
    print('Error in getReactionList: $e');
    throw e;
  }
}

Future<StoryReactionModel> saveStoryReaction({required int storyId, required int reactionId}) async {
  Map request = {"story_id": storyId, "reaction_id": reactionId};
  return StoryReactionModel.fromJson(await handleResponse(
      await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.storiesEndPoint}/${APIEndPoint.saveStoryReactionEndPoint}?story_id=${storyId}?reaction_id=${reactionId}', method: HttpMethod.POST, request: request)));
}

Future<List<UserStoryReactionList>> getStoryReactions({required int storyId}) async {
  try {
    Iterable it = await handleResponse(await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.storiesEndPoint}/${APIEndPoint.getUserStoryReactionEndPoint}?story_id=$storyId'));
    return it.map((e) => UserStoryReactionList.fromJson(e)).toList();
  } catch (e) {
    log('Error fetching story reactions: $e');
    return [];
  }
}
...
/// Highlight List
Future<List<StoryHighlight>> getHighlightStoryList() async {
  Iterable it = await handleResponse(await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.storiesEndPoint}/${APIEndPoint.highlightStoryListEndPoint}', method: HttpMethod.GET));

  return it.map((e) => StoryHighlight.fromJson(e)).toList();
}

///Create Highlight story

Future<CommonMessageResponse> createHighlightStory({required Map request}) async {
  return CommonMessageResponse.fromJson(
    await handleResponse(await buildHttpResponse('${APIEndPoint.createHighlightEndPoint}', method: HttpMethod.POST, request: request)),
  );
}
...
Future<List<ProductListModel>> getProductsList({int page = 1, int? categoryId, String? orderBy, String? searchText}) async {
  ...
  if (categoryId != null) {
    it = await handleResponse(
      await buildHttpResponse(
        '${APIEndPoint.productsList}?category=$categoryId${searchText.validate().isNotEmpty ? '&search=$searchText' : ''}&page=$page&per_page=$PER_PAGE${orderBy != null ? '&orderby=$orderBy' : ''}',
        passParameters: true,
        passToken: false,
      ),
    );
...
Future<List<RelatedProductList>> getRelatedProducts({required int productId}) async {
  try {
    Iterable it = await handleResponse(
      await buildHttpResponse('${APIEndPoint.relatedProducts}?product_id=$productId', passParameters: true, passToken: false),
    );
    return it.map((e) => RelatedProductList.fromJson(e)).toList();
  } catch (e) {
    log(e);
    throw e;
  }
...
Future<List<RecentlyViewedProductList>> getRecentlyViewedProductList() async {
  try {
    final response = await buildHttpResponse('${APIEndPoint.recentlyViewedProducts}', method: HttpMethod.GET, passParameters: true, passToken: true);

    final RecentlyViewedProductModel model = RecentlyViewedProductModel.fromJson(json.decode(response.body));
...
/// use this to remove single cart item from  the cart
...
/// use this to remove all cart item from  the cart
Future<CommonMessageResponse> clearCart() async {
  return CommonMessageResponse.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.clearCart}', method: HttpMethod.POST, request: {"user_id": userStore.loginUserId})));
}

Future<List<PaymentModel>> getPaymentMethods() async {
  Map request = {"enabled": true};

  Iterable it = await handleResponse(await buildHttpResponse('${APIEndPoint.getPaymentMethods}', request: request, passHeaders: false, passParameters: true));
...
Future<TopicModel> getTopicDetail({required int topicId, int page = 1}) async {
  ...
  final response = await handleResponse(await buildHttpResponse(
    getEndPoint(
      endPoint: '${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.forumsEndPoint}/${APIEndPoint.topicsEndPoint}/${APIEndPoint.detailsEndPoint}/$topicId',
      params: params,
    ),
    method: HttpMethod.GET,
  ));

  return TopicModel.fromJson(response as Map<String, dynamic>);
}
...
Future<List<Albums>> getAlbums({
  ...
  List<String> params = [];
  /* if (userId == userStore.loginUserId.toInt())*/
...
Future<List<InviteData>> getSearchResults({required int groupId, required String searchParam}) async {
  final jsonResponse = await handleResponse(await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.groupsEndpoint}/${APIEndPoint.invitesEndPoint}/$groupId?per_page=10&page=1&search_terms=$searchParam', passToken: false));
  try {
    final List<dynamic> inviteList = jsonResponse;
    final List<InviteData> invites = inviteList.map((inviteJson) => InviteData.fromJson(inviteJson)).toList();

    return invites;
  } catch (e) {
    print("Error parsing invites: $e");
    return [];
  }
}

//endregion
Future<WpCommentModel> replyCommentBlog({required int blogId, String? content, required int parentId}) async {
  Map request = {"post": blogId, "content": content, "parent": parentId};
  return WpCommentModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.blogCommentReply}', method: HttpMethod.POST, request: request)));
}
