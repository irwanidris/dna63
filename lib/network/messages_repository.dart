...
Future<SendMessageResponse> sendMessage({required int threadId, required String message, int? messageId, List<int>? fileIds, String? tmpId}) async {
  Map meta = messageId != null ? {"reply_to": messageId} : {};

  Map request = {"message": message.isNotEmpty ? message : null, "meta": meta, "files": fileIds, "message_id": messageId, "tmpId": tmpId};
...
Future<void> unblockMember({required int userId}) async {
  Map request = {"user_id": userId};
  await handleResponse(await buildHttpResponse('${MessageAPIEndPoint.unblockMember}/$userId/unblock', method: HttpMethod.POST, request: request));
}
...
Future<void> promoteAsAdmin({required int userId, required int groupId}) async {
  Map request = {"user_id": userId};
  await buildHttpResponse("${MessageAPIEndPoint.promoteAsAdmin}/$groupId/makeModerator", method: HttpMethod.POST, request: request);
}

Future<void> demoteFromAdmin({required int userId, required int groupId}) async {
  Map request = {"user_id": userId};
  await buildHttpResponse("${MessageAPIEndPoint.promoteAsAdmin}/$groupId/unmakeModerator", method: HttpMethod.POST, request: request);
}
...
Future<Messages> getMessage({required int messageId}) async {
  return Messages.fromJson(await handleResponse(await buildHttpResponse("${MessageAPIEndPoint.message}/$messageId")));
}
...
Future<void> getChatReactionList() async {
  Iterable it = await handleResponse(await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${MessageAPIEndPoint.messagesEndPoint}/${MessageAPIEndPoint.emojisEndPoint}/${MessageAPIEndPoint.reactionsEndPoint}'));
  appStore.setEmojiList(it.map((e) => Emojis.fromJson(e)).toList());
}
...
Future<void> demoteAsAdmin({required int userId, required int groupId}) async {
  Map request = {"user_id": userId};
  await handleResponse(await buildHttpResponse('${MessageAPIEndPoint.promoteAsAdmin}/$groupId/unmakeModerator', method: HttpMethod.POST,request: request));
}
