...
Future<void> sendMultiPartRequest(MultipartRequest multiPartRequest, {Function(dynamic)? onSuccess, Function(dynamic)? onError, bool hideCoverImageError = false}) async {
  ...
  if (response.statusCode.isSuccessful()) {
    if (body is Map) {
      if (body['success'] == false && (body['data'] != null)) {
        ...
        onSuccess?.call(response.body);
      } else if (body['result'] == false && body['error'].toString().isEmpty) {
    ...
    try {
      ...
      if (body is Map) {
        if (body['success'] == false && body['data'] != null) {
          ...
          /// this below condition is only for the Edit profile cover image issue. Do not use for other conditions
          if (hideCoverImageError &&
              response.statusCode ==
                  400 /*&& body['message'] == "You have selected an image that is smaller than recommended. For better results, make sure to upload an image that is larger than 1920px wide, and 400px tall."*/) {
            onSuccess?.call(response.body);
          } else {
            toast('$errorMessage');
            throw errorMessage;
          }
        } else if (body.containsValue("is too large! Please upload file up to 2 MB.")) ;
        {
          toast("${body['error']}");
        }
      } else {
