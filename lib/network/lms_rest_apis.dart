...
Future<CourseListModel> getCourseById({required int courseId}) async {
  return CourseListModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.courses}/$courseId',passToken: true)));
}
...
Future<List<LmsPaymentModel>> getLmsPayments() async {
  final response = await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.learnPressEndPoint}/${APIEndPoint.paymentsEndPoint}/${APIEndPoint.methodsEndPoint}');
  final data = await handleResponse(response);
  try {
    if (data is List) {
      return data.map((e) => LmsPaymentModel.fromJson(e as Map<String, dynamic>)).toList();
    } else if (data is Map<String, dynamic> && data['data'] != null) {
      final List items = data['data'] as List;
      return items.map((e) => LmsPaymentModel.fromJson(e as Map<String, dynamic>)).toList();
    }
  } catch (e) {
...
Future<LmsOrderModel> lmsPlaceOrder({
  ...
  return LmsOrderModel.fromJson(await handleResponse(
    await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.learnPressEndPoint}/${APIEndPoint.ordersEndPoint}', request: request, method: HttpMethod.POST),
  ));
}
...
Future<List<CourseOrders>> courseOrders({required int page}) async {
  Iterable it = await handleResponse(await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.learnPressEndPoint}/${APIEndPoint.ordersEndPoint}?page=$page&per_page=$PER_PAGE', method: HttpMethod.GET));

  return it.map((e) => CourseOrders.fromJson(e)).toList();
}

Future<LmsOrderModel> getOrderDetails({required int id}) async {
  return LmsOrderModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.socialVApiEndPoint}/${APIEndPoint.learnPressEndPoint}/${APIEndPoint.ordersEndPoint}/$id', method: HttpMethod.GET)));
}
