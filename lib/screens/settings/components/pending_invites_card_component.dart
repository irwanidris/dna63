    ...
    return Observer(builder: (context) {
      return GestureDetector(
        onLongPress: () {
          ...
          if (widget.isChecked) {
            messageStore.selectedInviteRequests.add(widget.inviteList.id);
            appStore.setMultiSelect(true);
          } else {
            messageStore.selectedInviteRequests.remove(widget.inviteList.id);
            if (messageStore.selectedInviteRequests.isEmpty) {
        ...
        onTap: () {
          if (appStore.isMultiSelect) {
            if (messageStore.selectedInviteRequests.contains(widget.inviteList.id)) {
              messageStore.selectedInviteRequests.remove(widget.inviteList.id);
              if (messageStore.selectedInviteRequests.isEmpty) {
                appStore.setMultiSelect(false);
                messageStore.isChecked = false;
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: messageStore.selectedInviteRequests.contains(widget.inviteList.id) ? greenColor.withAlpha(20) : context.cardColor, borderRadius: BorderRadius.circular(commonRadius)),
          child: Column(
                ...
                children: [
                  Container(
                    ...
                    child: Image.asset(
                      ...
                      .onTap(() {
                        if (messageStore.selectedInviteRequests.isEmpty) widget.callback?.call(widget.inviteList.id.validate().toInt(), true);
                      })
