...
class LoginInComponent extends StatefulWidget {
  ...
  LoginInComponent({this.callback, this.deepLink});
...
class _LoginInComponentState extends State<LoginInComponent> {
  ...
  void init() async {
    if (appStore.doRemember) {
      nameCont.text = userStore.loginName;
      passwordCont.text = userStore.password;
...
/*  Future<Map<String, dynamic>?> showPhoneNumberDialog(BuildContext context, Map loginReq) async {
        ...
        return Dialog(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Icon(Icons.phone_android, size: 40, color: context.primaryColor),
                16.height,
                Text("Add your phone number", style: boldTextStyle(size: 18)),
                16.height,
                Text(
                  "This step is optional. You can skip if you want.",
                ...
                AppTextField(
                  ...
                  decoration: inputDecoration(context, label: "Phone Number (Optional)"),
                ),
                24.height,
                Row(
                  ...
                  children: [
                    AppButton(
                      ...
                      onTap: () {
                        finish(context); // Return original request
                      },
                    ),
                    AppButton(
                      padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
...
/*  addPhoneNumber() {
    ifNotTester(() async {
      if (nameCont.text.isNotEmpty && nameCont.text != userStore.loginFullName) {
        appStore.setLoading(true);
        Map request = {"phone": phoneController.text.trim()};
        await updatePhoneNumber(request: request).then((value) {
          toast(language.phoneNumberAddedSuccessfully, print: true);
          appStore.setLoading(false);
          finish(context);
        }).catchError((e) {
          appStore.setLoading(false);
  ...
  void googleSignIn() async {
    ...
    await service.signInWithGoogle().then((res) async {
      ...
      /*   var updatedReq = await showPhoneNumberDialog(context, res);
      if (updatedReq != null) {
  ...
  void otpSignIn() async {
    ...
    OTPLoginScreen(deepLink: widget.deepLink).launch(context);
  }
