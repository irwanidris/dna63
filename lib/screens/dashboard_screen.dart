...
class _DashboardScreenState extends State<DashboardScreen> with TickerProviderStateMixin {
  ...
  @override
  void initState() {
    appStore.setDashboardIndex(widget.index ?? 0);
    ...
    appFragments.addAll([
      ...
      if (appStore.showForums && appStore.pluginStatus.bbpress.getBoolInt()) ForumsFragment(controller: _controller),
    ...
    tabController = TabController(
      length: appStore.showForums ? 5 : 4,
      vsync: this,
      initialIndex: widget.index ?? 0, // 👈 important
    );
    tabController.addListener(
      () {
        tabController.index == 0 ? homeFragStore.mHomePage = 1 : homeFragStore.mHomePage = 0;
      },
    );
  ...
  Future<void> getDashboard() async {
    await getDashboardDetails().then((value) {
      ...
      appStore.suggestedGroupsList.addAll(value.suggestedGroups.validate());
    }).catchError((e) {
      log(e.toString());
      toast(e.toString());
    });
          ...
          builder: (context) {
            return Scaffold(
                body: SafeArea(
                  child: Stack(
                    children: [
                          ...
                          Theme(
                            ...
                            child: SliverAppBar(
                              ...
                              actions: [
                                ...
                                if (appStore.showShop && appStore.showWoocommerce)
                                  IconButton(
                                    onPressed: () {
                                    ...
                                    splashColor: Colors.transparent,
                                    highlightColor: Colors.transparent,
                                    icon: Image.asset(ic_bag, height: 24, width: 24, fit: BoxFit.fitWidth, color: context.iconColor),
                                  ),
                                IconButton(
                                  highlightColor: Colors.transparent,
                                  splashColor: Colors.transparent,
                              ...
                              bottom: TabBar(
                                ...
                                tabs: [
                                  ...
                                  if (appStore.showForums && appStore.pluginStatus.bbpress.getBoolInt())
                                    Tooltip(
                      ...
                      // Home Fragment Loader
                      Observer(
                        builder: (_) {
                          if (appStore.isLoading) {
                            if (homeFragStore.mHomePage > 1) {
                              return Positioned(
                                bottom: 10,
                                left: 0,
                                right: 0,
                            ...
                            } else if (homeFragStore.mHomePage == 0) {
                              return Offstage();
                            } else {
                              return LoadingWidget(isBlurBackground: false).paddingTop(context.height() * 0.2).center();
                            }
                          } else {
                            return Offstage();
                          }
                      ...
                      Observer(
                        ...
                        builder: (_) => LoadingWidget(isBlurBackground: false).center().paddingTop(context.height() * 0.2).visible(
                              profileFragStore.mProfilePage == 1 && appStore.isLoading && tabController.index == 4,
                            ),
                      ),
                    ],
                  ),
                ...
                floatingActionButton: appFragments[appStore.currentDashboardIndex].runtimeType == NotificationFragment(controller: _controller).runtimeType
                    ...
                    : appStore.pluginStatus.bpBetterMessages == 1
                        ? Stack(
                            clipBehavior: Clip.none,
                            alignment: Alignment.center,
                            children: [
                              FloatingActionButton(
                                onPressed: () {
                                  ...
                                  if (pmpStore.privateMessaging) {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(settings: RouteSettings(name: 'MessageScreen'), builder: (_) => MessageScreen()),
                                    );
                                  } else {
