...
class StoryPageState extends State<StoryPage> {
  ...
  /// Fetch the list of reactions
  Future<void> fetchReactionList() async {
    appStore.isLoading = true;
    try {
      List<ReactionList> tempList = [];
      tempList = await getReactionList(reactionList: tempList);

      runInAction(() {
        homeFragStore.reactionList.clear();
        homeFragStore.reactionList.addAll(tempList);
      });
    } catch (e) {
      print('Error fetching reaction list: $e');
    } finally {
      appStore.isLoading = false;
    }
  }

  /// Handle the reaction of a story
  Future<void> handleStoryReaction(int storyId, int reactionId) async {
    if (homeFragStore.isStoryReacted) return;
    try {
      homeFragStore.isStoryReacted = true;
      final response = await saveStoryReaction(
        storyId: storyId,
        reactionId: reactionId,
      );
      runInAction(() {
        if (response.status ?? false) {
          if (response.isReacted == 1) {
            Map<int, int> newReactions = Map<int, int>.from(homeFragStore.storyReactions);
            newReactions[storyId] = reactionId;
            homeFragStore.storyReactions = newReactions;
          } else {
            Map<int, int> newReactions = Map<int, int>.from(homeFragStore.storyReactions);
            newReactions.remove(storyId);
            homeFragStore.storyReactions = newReactions;
          }
        }
      });
      ...
      homeFragStore.isStoryReacted = false;
    }
  }
            ...
            return Stack(
              ...
              children: [
                Container(
                  ...
                  child: Observer(
                    builder: (context) {
                      final currentStoryId = widget.stories[userIndex].items.validate()[currentStoryIndex].id.validate();

                      return HorizontalList(
                        padding: EdgeInsets.zero,
                        itemCount: homeFragStore.reactionList.length,
                        itemBuilder: (context, index) {
                          final reaction = homeFragStore.reactionList[index];

                          return Observer(
                            builder: (_) {
                              final isReacted = homeFragStore.storyReactions[currentStoryId] == reaction.id.validate();
                                ...
                                children: [
                                  Container(
                                    ...
                                    decoration: boxDecorationDefault(
                                      ...
                                      image: reaction.image?.url.validate().isNotEmpty != null ? DecorationImage(image: NetworkImage(reaction.image!.url!), fit: BoxFit.cover) : null,
                                    ),
                                    child: reaction.image?.url == null
                                        ? Container(
                                      decoration: boxDecorationDefault(
                                        shape: BoxShape.circle,
                                    ...
                                    handleStoryReaction(currentStoryId, reaction.id.validate());
                                  }),
                                  if (isReacted)
                                    Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      if (story.storyLink.validate().isNotEmpty)
                        Container(
                          padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                          decoration: BoxDecoration(color: Colors.black.withValues(alpha: 0.6), borderRadius: radius(defaultRadius)),
