...
import 'new_highlight_screen_component.dart';
...
class StoryHighlightsComponent extends StatefulWidget {
  ...
  StoryHighlightsComponent({this.highlightsList, this.avatarImage, this.showAddOption = true, this.callback});
...
class _StoryHighlightsComponentState extends State<StoryHighlightsComponent> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        if (!widget.showAddOption && widget.highlightsList.validate().isNotEmpty) Text(language.storyHighlights, style: boldTextStyle()).paddingSymmetric(horizontal: 16),
        Align(
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              ...
              children: [
                      ...
                      GestureDetector(
                        ...
                        onTap: () async {
                          NewHighlightScreen(
                            ...
                            onCreated: () {
                              widget.callback?.call(); // refresh highlights list
                            },
                          ).launch(context);
                        },
                      ),
                ...
                if (widget.highlightsList.validate().validate().isNotEmpty)
                  HorizontalList(
                    padding: EdgeInsets.only(left: widget.showAddOption ? 0 : 8, top: 8, bottom: 8, right: 8),
                    spacing: 4,
                    itemCount: widget.highlightsList.validate().length,
                    itemBuilder: (context, index) {
                      HighlightStoriesModel story = widget.highlightsList.validate()[index];
                      return InkWell(
                        ...
                        onTap: () {
                          HighlightStoryPage(
                            ...
                            initialIndex: index,
                            stories: widget.highlightsList.validate(),
                            callback: () {
                        ...
                        onLongPress: () async {
                          if (widget.showAddOption && appStore.storyActions.validate().isNotEmpty)
                            showDialog(
                              context: context,
                              builder: (BuildContext context) {
                                return Dialog(
                                  ...
                                  child: Column(
                                    ...
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      if (appStore.storyActions.validate().any((element) => element.action == StoryHighlightOptions.delete))
                                        TextIcon(
                                      ...
                                      Divider(),
                                      if (appStore.storyActions.validate().any((element) => element.action == StoryHighlightOptions.trash))
                                        TextIcon(
