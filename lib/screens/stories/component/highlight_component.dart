    ...
    return Stack(
      children: [
        Observer(builder: (_) {
            ...
            itemBuilder: (ctx, index) {
              ...
              return InkWell(
                onTap: () {
                  HighlightStoryPage(
                    showDelete: true,
                    avatarImage: userStore.loginAvatarUrl,
                    initialIndex: index,
          ...
          ).visible(widget.highlightList.isNotEmpty);
        }),
        Observer(builder: (_) {
          return Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ...
              TextButton(
                style: ButtonStyle(
                  ...
                  side: WidgetStateProperty.all(BorderSide(color: appColorPrimary.withValues(alpha: 0.5))),
                ),
                onPressed: () {
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    4.width,
                    Text(language.addYourHighlight, style: primaryTextStyle(size: 14)),
                  ],
                ),
              ),
            ],
          ).visible(widget.highlightList.isEmpty && !appStore.isLoading && widget.status == StoryHighlightOptions.publish).center();
        }),
        Observer(builder: (_) {
          return NoDataWidget(
            imageWidget: NoDataLottieWidget(),
            title: language.noDataFound,
          ).center().visible(widget.highlightList.isEmpty && !appStore.isLoading && (widget.status == StoryHighlightOptions.draft || widget.status == StoryHighlightOptions.trash));
        }),
        Observer(builder: (_) {
