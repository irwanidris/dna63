...
class _RequestComponentState extends State<RequestComponent> with AutomaticKeepAliveClientMixin {
  GroupStore requestComponentVars = GroupStore();
  ...
  Future<void> getRequests() async {
    await getGroupMembershipRequest(groupId: widget.groupId.validate(), page: mPage).then((value) {
      if (mPage == 1) requestComponentVars.requestList.clear();

      mIsLastPage = value.length != 20;
      requestComponentVars.requestList.addAll(value);
      requestComponentVars.isError = false;
      appStore.setLoading(false);
    }).catchError((e) {
      requestComponentVars.isError = true;
      appStore.setLoading(false);
      toast(e.toString());
    });
  }
        ...
        child: Stack(
          ...
          children: [
            Observer(builder: (context) {
              return requestComponentVars.isError && !appStore.isLoading
                  ? SizedBox(
                      height: context.height() * 0.88,
                      child: NoDataWidget(
                        imageWidget: NoDataLottieWidget(),
                        title: requestComponentVars.isError ? language.somethingWentWrong : language.noDataFound,
                        onRetry: () {
                          onRefresh();
                        },
            ...
            Observer(builder: (context) {
              return requestComponentVars.requestList.isEmpty && !requestComponentVars.isError && !appStore.isLoading
                  ? SizedBox(
                      height: context.height() * 0.88,
                      child: NoDataWidget(
                        imageWidget: NoDataLottieWidget(),
            ...
            /// list widget
            Observer(builder: (context) {
              return requestComponentVars.requestList.isNotEmpty && !requestComponentVars.isError
                  ? AnimatedListView(

                      padding: EdgeInsets.only(left: 16, right: 16, bottom: 50),
                      itemCount: requestComponentVars.requestList.length,
                      slideConfiguration: SlideConfiguration(
                        delay: 80.milliseconds,
                        verticalOffset: 300,
                      ),
                      itemBuilder: (ctx, index) {
                        GroupRequestModel member = requestComponentVars.requestList[index];
                        return Row(
