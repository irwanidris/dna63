...
class _AlbumUploadScreenState extends State<AlbumUploadScreen> {
  ...
  void initState() {
    super.initState();
    galleryStore.mediaList.clear();
    if (widget.fileType != null) {
      appStore.setSelectedMedia(appStore.allowedMedia
          .validate()
          .firstWhere((element) => element.component == (widget.groupId != null ? Component.groups : Component.members))
          .allowedTypes
          .validate()
    ...
    if (file != null) {
      if (file == FileTypes.CAMERA) {
        ...
        await getImageSource(isCamera: true, isVideo: appStore.selectedAlbumMedia!.type == MediaTypes.video).then((value) {
          ...
          galleryStore.mediaList.add(PostMedia(file: value));
        }).catchError((e) {
        ...
        getMultipleFiles(mediaType: appStore.selectedAlbumMedia!, postingInComponent: widget.groupId != null ? Component.groups : Component.members).then((value) {
          ...
          value.forEach((element) {
            galleryStore.mediaList.add(PostMedia(file: element));
          });
        }).catchError((e) {
          appStore.setLoading(false);
          log('Error: ${e.toString()}');
        });
        log('MediaList: ${galleryStore.mediaList.length}');
      }
    }
  }
  ...
  void onUpload() async {
    ifNotTester(() async {
      if (galleryStore.mediaList.isEmpty) {
        toast(language.addPostContent);
      } else {
        log(widget.albumId);
        appStore.setLoading(true);
        await uploadMediaFiles(groupId: widget.groupId, count: galleryStore.mediaList.length, galleryId: widget.albumId, media: galleryStore.mediaList).then(
          (value) async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(language.uploadAlbum, style: boldTextStyle(size: 20)),
        elevation: 0,
        centerTitle: true,
        leading: IconButton(
            ...
            SingleChildScrollView(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    if (widget.fileType == null)
                      Text("2. ${language.addMediaFile}", style: primaryTextStyle(color: appStore.isDarkMode ? bodyDark : bodyWhite, size: 18)).paddingSymmetric(horizontal: 16),
                    Stack(
                      children: [
                        DottedBorderWidget(
                    ...
                    Observer(builder: (context) {
                      return ShowSelectedMediaComponent(
                        mediaList: galleryStore.mediaList,
                        mediaType: appStore.selectedAlbumMedia!,
                        videoController: List.generate(galleryStore.mediaList.length, (index) {
                          return VideoPlayerController.file(galleryStore.mediaList[index].file!);
                        }),
                      ).visible(galleryStore.mediaList.isNotEmpty);
                    }),
                    8.height,
                    Align(
