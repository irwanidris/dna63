...
class ProfileFieldComponent extends StatefulWidget {
  final ProfileFieldsModel field;
  int count;
  final bool isFromSignUP;

  ProfileFieldComponent({required this.field, required this.count, this.isFromSignUP = false});
...
class _ProfileFieldComponentState extends State<ProfileFieldComponent> {
  UserStore profileFieldComponentVars = UserStore();
  ...
  Future<void> init() async {
    if (widget.field.type == FieldType.selectBox) {
      if (widget.field.options.validate().isNotEmpty && widget.field.value.validate().isEmpty) {
        ...
        profileFieldComponentVars.selectedValue = widget.field.options.validate().firstWhere((element) => element.name == widget.field.value);
      }
      ...
      if (widget.field.label == 'Name' && widget.field.value.isEmpty) {
        if (!widget.isFromSignUP) controller.text = userStore.loginFullName;
      } else {
        if (!widget.isFromSignUP) controller.text = widget.field.value.validate();
      }
    }
  }
            ...
            Observer(builder: (context) {
              return IgnorePointer(
                ...
                child: DropdownButton<OptionField>(
                  ...
                  value: profileFieldComponentVars.selectedValue,
                  ...
                  onChanged: (OptionField? newValue) {
                    profileFieldComponentVars.selectedValue = newValue!;
                    group.fields[group.fields.indexOf(widget.field)].value = profileFieldComponentVars.selectedValue!.name.validate();
                    isDetailChange = true;
                  },
