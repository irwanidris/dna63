...
class _RequestFollowWidgetState extends State<RequestFollowWidget> {
  ProfileMenuStore requestFollowWidget = ProfileMenuStore();

  @override
  void initState() {
    super.initState();
    requestFollowWidget.friendshipStatus = widget.friendshipStatus;
  }
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (context) {
                ...
                showDialog(
                  ...
                  builder: (BuildContext context) {
                    return UnblockMemberDialog(
                      ...
                      callback: () {
                        requestFollowWidget.friendshipStatus = Friendship.notFriends;
        ...
        } else if (requestFollowWidget.friendshipStatus == Friendship.notFriends && appStore.displayFriendRequestBtn) {
          return AppButton(
            shapeBorder: RoundedRectangleBorder(borderRadius: radius(4)),
            ...
            onTap: () async {
              if (pmpStore.sendFriendRequest) {
                ifNotTester(() async {
                  requestFollowWidget.friendshipStatus = Friendship.pending;
                  Map request = {"initiator_id": userStore.loginUserId, "friend_id": widget.memberId.toString().validate()};
        ...
        } else if (requestFollowWidget.friendshipStatus == Friendship.pending) {
          return AppButton(
            shapeBorder: RoundedRectangleBorder(borderRadius: radius(4)),
        ...
        } else if (requestFollowWidget.friendshipStatus == Friendship.awaitingResponse) {
          return Row(
            ...
            children: [
              AppButton(
                ...
                onTap: () async {
                  ifNotTester(() async {
                    requestFollowWidget.friendshipStatus = Friendship.isFriend;
                    await acceptFriendRequest(id: widget.memberId.validate()).then((value) {
                      widget.callback?.call();
                    }).catchError((e) {
                      requestFollowWidget.friendshipStatus = Friendship.awaitingResponse;
                      toast(e.toString(), print: true);
                    });
                  });
                },
        ...
        } else if (requestFollowWidget.friendshipStatus == Friendship.isFriend) {
          return AppButton(
            shapeBorder: RoundedRectangleBorder(borderRadius: radius(4)),
