...
class _WriteMessageComponentState extends State<WriteMessageComponent> {
  MessageStore writeMessageCompVars = MessageStore();
  ...
  void _loadDraft() async {
    String draftKey = 'draft_${widget.threadId}';
    messageStore.draftMessage = "";

    messageContentKey.currentState?.controller?.text = messageStore.messageMetaData[draftKey] ?? "";
    setState(() {});
  }
      ...
      if (file == FileTypes.CAMERA) {
        await getImageSource(isCamera: true).then((value) {
          ...
          MessageMediaScreen(
            ...
            writeMessageCompVars.showReply = false;
  ...
  void dispose() {
    String draftKey = 'draft_${widget.threadId}';
    String text = messageStore.draftMessage;

    if (text.isNotEmpty) {
      if (text.contains("Draft:")) {
        text = text.replaceAll("Draft:", "");
      }
      messageStore.messageMetaData[draftKey] = "$text";
    } else {
      messageStore.messageMetaData.remove(draftKey);
    }
      ...
      child: Observer(builder: (context) {
        return Column(
          ...
          children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(widget.isEditMessage.validate() ? language.editingMessage : language.replyToMessage, style: boldTextStyle(size: 14), overflow: TextOverflow.ellipsis, maxLines: 1),
                        ],
                      ),
                    ),
                    IconButton(
                      onPressed: () {
                        messageContentKey.currentState?.controller?.text = "";
            ...
            Row(
              children: [
                if (widget.canUpload && widget.replyMessage == null)
                  IconButton(
                    icon: cachedImage(ic_camera_post, height: 24, width: 24, fit: BoxFit.cover, color: context.iconColor),
                    onPressed: () {
                      pickFiles();
                    },
                  ),
                FlutterMentions(
                  ...
                  onChanged: (text) {
                    messageStore.draftMessage = text;
                  ...
                  onPressed: () {
                    ifNotTester(() async {
                      if (messageContentKey.currentState!.controller!.text.isNotEmpty) {
                        if (messageContentKey.currentState!.controller!.text.validate().contains("Draft: ")) {
                          messageContentKey.currentState!.controller!.text = messageContentKey.currentState!.controller!.text.validate().replaceAll("Draft: ", "");
                        } else {
                          messageContentKey.currentState!.controller!.text = messageContentKey.currentState!.controller!.text.validate().replaceAll("Draft: ", "");
                        }
                        ...
                        if (widget.isEditMessage.validate() && widget.replyMessage != null) {
                          editMessage(
                            ...
                            if (!appStore.isWebsocketEnable) {
                              Messages message = Messages(
                                ...
                                senderId: userStore.loginUserId.validate().toInt(),
                                messageId: widget.replyMessage!.messageId.validate(),
                              );
                          ...
                          if (widget.replyMessage != null) {
                            ...
                            if (appStore.isWebsocketEnable) {
                              String message =
                                  '423["${SocketEvents.v3fastMsg}",${widget.threadId},"${messageContentKey.currentState!.controller!.text}",{"user_id":0,"name":"${userStore.loginFullName}","avatar":"${userStore.loginAvatarUrl}"},"${DateTime.now().toUtc()}"]';
                              ...
                              Messages message = Messages(
                                threadId: widget.threadId,
                                dateSent: DateFormat('yyyy-MM-dd').format(DateTime.now()).toString(),
                                message: tempText.validate(),
                                senderId: userStore.loginUserId.validate().toInt(),
                                tmpId: tempId,
                                lastUpdate: createMessageTime(),
                                createdAt: createMessageTime(),
                              );
                              //   widget.onAddMessage?.call(message);
                              sendMessage(
                                threadId: widget.threadId,
