...
class _SetChatWallpaperScreenState extends State<SetChatWallpaperScreen> {
  @override
  void initState() {
    if (widget.wallpaperUrl != null) {
      messageStore.wallpaperUrl = widget.wallpaperUrl;
    }
    messageStore.isGeneralSetting = widget.isGeneralSetting;
    super.initState();
  }
  ...
  Future<void> setWallpaper({required File file}) async {
    ...
    saveChatBackground(
      id: messageStore.isGeneralSetting ? userStore.loginUserId : widget.threadId.validate().toString(),
      image: file,
      type: messageStore.isGeneralSetting ? "global" : "thread",
    ).then((value) {
  ...
  bool showDeleteButton() {
    if (messageStore.wallpaperFile != null || (messageStore.wallpaperUrl != null && messageStore.wallpaperUrl.validate().isNotEmpty)) {
      if (messageStore.wallpaperUrl.validate() == messageStore.globalChatBackground && !messageStore.isGeneralSetting && messageStore.wallpaperFile == null) {
        return false;
      } else {
        return true;
      }
  ...
  void dispose() {
    ...
    messageStore.wallpaperFile = null;
    messageStore.wallpaperUrl = null;
    super.dispose();
  }
            ...
            child: Stack(
              children: [
                Observer(builder: (context) {
                  return messageStore.wallpaperUrl != null && messageStore.wallpaperUrl.validate().isNotEmpty
                      ? cachedImage(
                          widget.wallpaperUrl,
                          height: context.height() * 0.6,
                ...
                Observer(builder: (context) {
                  return messageStore.wallpaperFile != null
                      ? Image.file(
                          File(messageStore.wallpaperFile!.path.validate()),
                          height: context.height() * 0.6,
                          width: context.width() * 0.8,
                          fit: BoxFit.cover,
                        ).cornerRadiusWithClipRRect(commonRadius)
