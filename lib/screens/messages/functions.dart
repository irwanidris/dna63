...
void handleSocketEvents(BuildContext context, String message) {
  ...
  if (event == SocketEvents.message) {
    ...
    if (threadOpened != null &&
        streamMessage.thread!.threadId == threadOpened) {
      LiveStream()
          .emit(ThreadMessageReceived, streamMessage.thread!.lastMessage);
    }
    ...
    unreads.threads!.keys.map((e) {
      messageStore.addUnReads(UnreadThreadModel(
          threadId: e.toInt(), unreadCount: unreads.threads![e]));
    }).toList();
    ...
    removeTypingUsers();
    typing.keys.map((e) {
      if (!messageStore.typingList
          .any((element) => element.threadId == e.toInt())) {
        messageStore.addTyping(UnreadThreadModel(
            threadId: e.toInt(), typingIds: typing[e].cast<String>()));
      } else {
        ...
        if (!inList) {
          messageStore.typingList.add(UnreadThreadModel(
              threadId: e.toInt(), typingIds: typing[e].cast<String>()));
        }
      }
    }).toList();
    ...
    if (threadOpened != null) {
      LiveStream()
          .emit(DeleteMessage, object.validate().replaceAll('"', '').toInt());
    }
  } else if (event == SocketEvents.v3fastMsg) {
    String? object = getStreamMessage(message);
    FastMessageModel streamMessage =
        FastMessageModel.fromJson(jsonDecode(object!));
    ...
    String? object = getStreamMessage(message);
    AbortMessageModel streamMessage =
        AbortMessageModel.fromJson(jsonDecode(object!));
...
String getChatDate({required String date}) {
  String formattedDate =
      DateFormat(DATE_FORMAT_3).format(DateTime.parse(date.substring(0, 10)));
...
// Add a helper function to handle PHP timestamps
...
String timeOfMessage(String dateSent) {
  try {
    final DateTime utcTime = DateTime.parse(dateSent).toUtc();
