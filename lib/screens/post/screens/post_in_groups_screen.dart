...
class _PostInGroupsScreenState extends State<PostInGroupsScreen> {
  ...
  Future<List<GroupModel>> getGroups() async {
    ...
    await getUserGroupList(
      groupType: GroupRequestType.userGroup,
      page: mPage,
      groupList: groupList,
      userId: userStore.loginUserId.validate().toInt(),
      lastPageCallback: (p0) {
        mIsLastPage = p0;
      },
    ).then((value) {
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      actions: [
        TextButton(
          onPressed: () {
            if (appStore.selectedIndex != null) {
              finish(
                context,
                PostInListModel(title: groupList[appStore.selectedIndex.validate()].name, id: groupList[appStore.selectedIndex.validate()].id),
              );
            } else {
              finish(context);
            }
                  ...
                  return Observer(builder: (context) {
                    return InkWell(
                      ...
                      child: Container(
                        ...
                        color: appStore.selectedIndex == index ? context.primaryColor.withAlpha(20) : context.scaffoldBackgroundColor,
                        child: TextIcon(
                          ...
                          spacing: 16,
                          expandedText: true,
                          suffix: appStore.selectedIndex == index ? Icon(Icons.check, color: context.primaryColor) : Offstage(),
