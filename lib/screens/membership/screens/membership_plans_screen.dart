...
class _MembershipPlansScreenState extends State<MembershipPlansScreen> {
  bool autoPlay = true;
  ProfileMenuStore membershipPlansScreenVars = ProfileMenuStore();
  ...
  Future<void> getPlansList() async {
    ...
    membershipPlansScreenVars.plans.clear();
    membershipPlansScreenVars.isError = false;
    await getLevelsList().then((value) async {
      membershipPlansScreenVars.plans.addAll(value.validate());
      appStore.setLoading(false);
    }).catchError((e) {
      membershipPlansScreenVars.isError = true;
      appStore.setLoading(false);
      ...
      child: Stack(
        ...
        children: [
          ...
          Observer(builder: (_) {
            ...
            ).center().visible(!appStore.isLoading && membershipPlansScreenVars.isError);
          }),
          ...
          Observer(builder: (_) {
            ...
            ).center().visible(!appStore.isLoading && !membershipPlansScreenVars.isError && membershipPlansScreenVars.plans.isEmpty);
          }),
            ...
            children: [
              Observer(builder: (context) {
                return CarouselSlider(
                  items: membershipPlansScreenVars.plans.map((plan) {
                    return Container(
                                      ...
                                      if (appStore.hasInAppSubscription) {
                                        if (plan.isFree.validate()) {
                                          if (appStore.inAppActiveSubscription.isNotEmpty) {
                                            ...
                                            if (appStore.isFreeSubscription) {
