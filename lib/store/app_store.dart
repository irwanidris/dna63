...
import '../../../utils/sv_reactions/sv_reaction.dart' as sv;
...
abstract class AppStoreBase with Store {
  ...
  @observable
  String wooConsumerSecret = getStringAsync(SharePreferencesKey.WOO_CONSUMER_SECRET);
  ...
  @observable
  ObservableList<SuggestedGroup> suggestedGroupsList = ObservableList();
  ...
  @observable
  ObservableList<PaypalPay> paypalPay = ObservableList();
  ...
  @observable
  ObservableList<GamipressAchievementTypes> gamipressRankEndPoints = ObservableList();

  @observable
  ObservableList<GamipressAchievementTypes> gamipressPointsEndPoints = ObservableList();
  ...
  /// sv_reactionBox vars
  @observable
  double reactionsBoxScale = 0;

  @observable
  double reactionsBoxItemScale = 1;

  ///svReaction button vars

  @observable
  sv.Reaction? selectedFlutterReaction;
  ...
  /// Sign in and signup Screens vars
  @observable
  int selectedSignInIndex = 0;

  @observable
  bool agreeTNC = false;

  /// OTP login Screen vars
  ...
  @action
  void setUserPremiumStatus(bool value) => userIsPremium = value;
  ...
  @action
  Future<void> setGamipressRankEndPointList(List<GamipressAchievementTypes> newList) async {
    gamipressRankEndPoints.clear();
    gamipressRankEndPoints.addAll(newList);
    await setValue(SharePreferencesKey.rankType, jsonEncode(newList.map((e) => e.toJson()).toList()));
  }

  @action
  Future<void> setGamipressPointsEndPointList(List<GamipressAchievementTypes> newList) async {
    gamipressPointsEndPoints.clear();
    gamipressPointsEndPoints.addAll(newList);
    await setValue(SharePreferencesKey.pointsType, jsonEncode(newList.map((e) => e.toJson()).toList()));
  }
  ...
  Future<void> setWooConsumerSecret(String val) async {
    ...
    await setValue(SharePreferencesKey.WOO_CONSUMER_SECRET, '$val', print: true);
  }
  ...
  @observable
  PluginModel pluginStatus = PluginModel.fromJson(getJSONAsync(SharePreferencesKey.pluginStatus));
  @action
  Future<void> setPluginStatus(PluginModel val, {bool isInitializing = false}) async {
    pluginStatus = val;
    setValue(SharePreferencesKey.pluginStatus, val.toJson());
  }

  @observable
  bool isMembershipEnable = getBoolAsync(SharePreferencesKey.showMembership);

  @action
  Future<void> setShowMembership(bool val) async {
    isMembershipEnable = val;
    setValue(SharePreferencesKey.membershipEnable, val, print: true);
  }

  @observable
  ObservableList<BlockedAccountsModel> blockedUsersList = ObservableList();
}
