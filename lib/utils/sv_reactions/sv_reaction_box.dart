...
class _ReactionsBoxState extends State<ReactionsBox> with TickerProviderStateMixin {
  ...
  AppStore reactionBoxStore = AppStore();
  ...
  void initState() {
    ...
    _scaleAnimation = startTween.animate(_scaleController)
      ..addListener(() {
        reactionBoxStore.reactionsBoxScale = _scaleAnimation.value;
      })
          ...
          Positioned(
            ...
            child: Observer(builder: (context) {
              return Transform.scale(
                scale: reactionBoxStore.reactionsBoxScale,
                child: Card(
                  color: widget.color,
...
class _ReactionsBoxItemState extends State<ReactionsBoxItem> with TickerProviderStateMixin {
  late AnimationController _scaleController;
  AppStore reactionBoxItemStore = AppStore();
  ...
  void initState() {
    ...
    _scaleAnimation = startTween.animate(_scaleController)
      ..addListener(() {
        reactionBoxItemStore.reactionsBoxItemScale = _scaleAnimation.value;

        if (reactionBoxItemStore.reactionsBoxItemScale == 1.3 && _overlayEntry == null) {
          _showTitle();
        } else if (reactionBoxItemStore.reactionsBoxItemScale == 1) {
          _hideTitle();
        }
      });
  }
  ...
  Widget build(BuildContext context) => IgnorePointer(
        ...
        child: Observer(builder: (context) {
          return Transform.scale(
            scale: reactionBoxItemStore.reactionsBoxItemScale,
            child: InkWell(
              onTap: () {
                _scaleController.reverse();
