...
extension StringExtension on String {
  String get nonBreaking {
    return replaceAll(RegExp(r'\s+'), ' ').replaceAll('&nbsp;', ' ').replaceAll(RegExp(r'\n\s*\n'), '\n').trim();
  }
}
...
class HtmlWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    String cleanedContent = postContent.validate().trim();

    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        if (cleanedContent.contains('youtube_url')) YoutubeComponent(postContent: cleanedContent),
        Html(
          data: cleanedContent.nonBreaking,
          shrinkWrap: true,
          // doNotRenderTheseTags: const {'br'},
          style: {
            "table": commonStyle(backgroundColor: color ?? transparentColor),
            "tr": commonStyle(border: Border(bottom: BorderSide(color: Colors.black45.withValues(alpha: 0.5)))),
            "th": commonStyle(backgroundColor: Colors.black45.withValues(alpha: 0.5)),
            "td": commonStyle(alignment: Alignment.center),
            'embed': commonStyle(passedColor: color ?? transparentColor, fontStyle: FontStyle.italic, fontWeight: FontWeight.bold, margin: Margins.zero),
            'strong': commonStyle(margin: Margins.zero),
            'a': commonStyle(passedColor: Colors.blue, fontWeight: FontWeight.bold, textDecoration: TextDecoration.none, margin: Margins.zero),
            'div': commonStyle(margin: Margins.zero),
            'figure': commonStyle(margin: Margins.zero),
            'h1': commonStyle(margin: Margins.only(bottom: 8)),
            'h2': commonStyle(margin: Margins.only(bottom: 8)),
            'h3': commonStyle(margin: Margins.only(bottom: 8)),
            'h4': commonStyle(margin: Margins.only(bottom: 8)),
            'h5': commonStyle(margin: Margins.only(bottom: 8)),
            'h6': commonStyle(margin: Margins.only(bottom: 8)),
            'p': commonStyle(
              textAlign: TextAlign.justify,
              margin: Margins.only(bottom: 8),
            ),
            'ol': commonStyle(margin: Margins.only(bottom: 8)),
            'ul': commonStyle(margin: Margins.only(bottom: 8)),
            'strike': commonStyle(margin: Margins.zero),
            'u': commonStyle(margin: Margins.zero),
            'b': commonStyle(margin: Margins.zero),
            'i': commonStyle(margin: Margins.zero),
            'hr': commonStyle(margin: Margins.symmetric(vertical: 8)),
            'header': commonStyle(margin: Margins.zero),
            'code': commonStyle(margin: Margins.zero),
            'data': commonStyle(margin: Margins.zero),
            'body': commonStyle(margin: Margins.zero),
            'big': commonStyle(margin: Margins.zero),
            'audio': commonStyle(margin: Margins.zero),
            'figcaption': commonStyle(margin: Margins.zero, padding: HtmlPaddings.zero, textAlign: TextAlign.center),
            'img': Style(width: Width(context.width()), padding: HtmlPaddings.zero, margin: Margins.only(bottom: 8), fontSize: FontSize(fontSize)),
            'li': commonStyle(
          ...
          extensions: [
            ...
            TagExtension(
              ...
              builder: (extensionContext) {
                if (extensionContext.innerHtml.contains('yout')) {
                  ...
                  String img = extensionContext.innerHtml.splitAfter('<a href="').splitBefore('"><img loading');
            ...
            TagExtension(
              ...
              builder: (extensionContext) {
                ...
                if (img.isNotEmpty && Uri.tryParse(img)?.hasAbsolutePath == true && Uri.tryParse(img)!.hasScheme) {
                  return CachedNetworkImage(
                    imageUrl: img,
                    width: context.width(),
                    fit: BoxFit.cover,
