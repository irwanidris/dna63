...
void onShareTap(BuildContext context) async {
  if (isAndroid) {
    SharePlus.instance.share(ShareParams(text: 'Download $APP_NAME app $playStoreBaseURL${await getPackageName()}'));
  } else {
    SharePlus.instance.share(ShareParams(text: 'Download $APP_NAME app $IOS_APP_LINK'));
  }
}
...
Future<List<File>> getMultipleFiles({required MediaModel mediaType, required String postingInComponent}) async {
  ...
  filePickerResult = await FilePicker.platform.pickFiles(
    allowMultiple: true, 
    type: type, 
    allowedExtensions: type == FileType.custom ? mediaType.allowedType.validate() : null
  );
  ...
  if (filePickerResult != null) {
    filePickerResult.files.forEach((element) {
      ...
      String fileExtension = element.path.validate().split("/").last.split(".").last.toLowerCase();
            bool isAccepted = false;
      
      if (isIOS && type == FileType.image) {
        isAccepted = true;
      } else if (fileExtension.isNotEmpty && (mediaType.allowedType!.any((e) => e.toLowerCase() == fileExtension) || fileExtension == 'heic')) {
        isAccepted = true;
      }
      if (isAccepted) {
...
String convertToAgo(String dateTime) {
  if (dateTime.isNotEmpty) {
    try {
      DateTime input = DateFormat(dateTime.contains('T') ? DATE_FORMAT_2 : DATE_FORMAT_1).parse(dateTime, false); // Changed 'true' to 'false' to parse in local timezone
      int timestamp = input.millisecondsSinceEpoch;
      return socialvFormatTime(timestamp);
    } catch (e) {
...
Future<List<MediaSourceModel>> getMultipleImages({List<String>? allowedTypeList}) async {
  ...
  filePickerResult = await FilePicker.platform.pickFiles(allowMultiple: true, type: allowedTypeList != null && allowedTypeList.isNotEmpty ? FileType.custom : FileType.any, allowedExtensions: allowedTypeList);
    ...
    for (var element in filePickerResult.files) {
      ...
      if (allowedTypeList.validate().isNotEmpty) {
        if (allowedTypeList.validate().contains(ext)) {
          ...
          toast('This file extension is not allowed.', print: true);
        }
      } else {
        toast("This file extension is not allowed.", print: true);
      }
...
const bool isIqonicProduct = true;
...
int createMessageTime() {
  // Return current timestamp in seconds (Unix timestamp)
...
Future<void> handleDeepLinking({required BuildContext context, required String deepLink, VoidCallback? callback, bool isNewTask = true}) async {
  if (appStore.isLoggedIn) {
    /// define the unique key of the app that contains in the deeplink URL
    if (deepLink.startsWith(DEEPLINK_KEY) || deepLink.startsWith('${DOMAIN_URL}/')) {
      var segments = deepLink.substring('${DEEPLINK_KEY}'.length).split('/');
      if (segments.length == 1 && segments[0].trim().isDigit() && int.tryParse(segments[0].trim()) != null) {
        ...
        if (segments.contains("activity")) {
          int postId = int.parse(segments[segments.length - 1].trim());
          SinglePostScreen(postId: postId).launch(context, isNewTask: isNewTask);
        } else {
          toast("Deep linking failed.");
        }
        if (segments.contains('blog')) {
...
class InvoiceDownloader {
  static String _getEndpoint(InvoiceType type, Map<String, String> params) {
    switch (type) {
      case InvoiceType.order:
        return '${APIEndPoint.orderInvoice}?order_id=${params['orderId']}';
      case InvoiceType.membership:
        return '${APIEndPoint.membershipInvoice}?membership_id=${params['membershipId']}&user_id=${params['userId']}';
      case InvoiceType.course:
        return '${APIEndPoint.courseInvoice}?course_id=${params['courseId']}&user_id=${params['userId']}';
    }
  }

  static String _generateFilename(InvoiceType type, Map<String, String> params) {
    String timestamp = DateTime.now().millisecondsSinceEpoch.toString();
    switch (type) {
      case InvoiceType.order:
        return 'invoice_${params['orderId']}_$timestamp.pdf';
      case InvoiceType.membership:
        return 'membership_invoice_${params['membershipId']}_${params['userId']}_$timestamp.pdf';
      case InvoiceType.course:
        return 'course_invoice_${params['courseId']}_${params['userId']}_$timestamp.pdf';
    }
  }
  ...
  static Future<File?> downloadInvoice({
    ...
    try {
      String invoiceUrl = _getEndpoint(type, params);

      var response = await handleResponse(
          await buildHttpResponse(invoiceUrl, method: HttpMethod.GET)
      );
      ...
      if (response is Map<String, dynamic>) {
        String? fileUrl = response['invoice_url'];
        String? base64Content = response['invoice_base64'];
        ...
        if (fileUrl != null) {
          ...
          invoiceBytes = fileResponse.bodyBytes;
        } else if (base64Content != null) {
          try {
            invoiceBytes = base64Decode(base64Content);
          } catch (e) {
            print('Error decoding base64 content: $e');
            return null;
          }
        } else if (directDownloadLink != null) {
          var linkResponse = await get(Uri.parse(directDownloadLink));
          if (linkResponse.statusCode != 200) {
            print('Failed to download file from direct link. Status code: ${linkResponse.statusCode}');
            return null;
          }
          invoiceBytes = linkResponse.bodyBytes;
        } else {
          print('No valid invoice data found in the response');
          return null;
        }
      } else if (response is List<int>) {
        invoiceBytes = response;
      } else {
      ...
      String fileName = _generateFilename(type, params);
      File file = File('${documentsDirectory.path}/$fileName');

      try {
        await file.writeAsBytes(invoiceBytes);
        print('${type.toString().split('.').last} Invoice saved successfully');
      ...
      print('Unexpected error in downloadInvoice: $e');
      return null;
    }
  }
}
